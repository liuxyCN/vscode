...
class ToggleEditModeAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, browserEditor = accessor.get(IEditorService).activeEditorPane): Promise<void> {
		...
		if (enabling) {
			const { BrowserEditorHtmlLayoutModeContribution } = await import('./browserEditorHtmlLayoutFeature.js');
			await browserEditor.getContribution(BrowserEditorHtmlLayoutModeContribution)?.cancelLayoutMode();
		}
